...
function build_survey(hl_survey) {          
    ...
    addStepNumbers(hl_survey);
        ...
        jQuery(hl_survey).find('.ghl-survey-form').each(function(){
            // check if progress bar is already appended
            if (jQuery(this).find('#progress_bar').length) {
                return; // exit early if progress bar already exists
            }
        ...
        jQuery(hl_survey).find('.ghl-question').each(function(){
            if (jQuery(this).attr('data-count')) {
                return; // exit early if progress cell already exists
            }
...
function addStepNumbers(hl_survey) {
    if (hl_survey.querySelector('.step')) {
        return;
    }
    if (hl_survey.querySelector('.ghl-step-num')) {
        return;
    }
